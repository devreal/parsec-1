...
static inline void parsec_heap_init(parsec_heap_t *heap, size_t comp_offset) {
    heap->top = NULL;
    heap->size = 0;
...
static inline void parsec_heap_fini(parsec_heap_t *heap) {
    (void)heap;
}

/** Return non-zero if the heap is empty. */
static inline int parsec_heap_is_empty(const parsec_heap_t *heap) {
    return (heap->size == 0);
}

/** Return the number of elements. */
static inline size_t parsec_heap_size(const parsec_heap_t *heap) {
    return heap->size;
}

/** View the maximum element without removing it. O(1). */
static inline parsec_list_item_t *parsec_heap_peek(const parsec_heap_t *heap) {
    return heap->top;
}
