...
parsec_hbbuffer_pop_best(parsec_hbbuffer_t *b, off_t priority_offset)
{
    ...
    if( best_elt != NULL ) {
        char tmp[MAX_TASK_STRLEN];
        if (priority_offset == offsetof(parsec_task_heap_t, priority)) {
                PARSEC_DEBUG_VERBOSE(20, parsec_debug_output, "HBB:\tFound best element %s in heap %p in local queue %p at position %d",
                        parsec_task_snprintf(tmp, MAX_TASK_STRLEN, (parsec_task_t*)((parsec_task_heap_t*)best_elt)->heap.top), best_elt,
                        b, best_idx);
        } else {
