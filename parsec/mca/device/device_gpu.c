...
void parsec_device_release_gpu_copy(parsec_device_gpu_module_t* gpu_device, parsec_data_copy_t *gpu_elem)
{
    ...
    if (0 != (gpu_elem->flags & PARSEC_DATA_FLAG_PARSEC_OWNED)) {
        ...
        parsec_data_t* data = gpu_elem->original;
        if (data != NULL) {
            /* have to take the lock to protect against double frees */
                             ...
                             parsec_gpu_task_t          *gpu_task )
{
    ...
    for( uint32_t i = 0; i < gpu_task->nb_flows  /* not this_task->task_class->nb_flows */; i++ ) {
        ...
        {
            parsec_data_t *pop_original = gpu_copy->original;
            parsec_gpu_data_copy_t *pop_cpu_copy = (pop_original != NULL) ?
                pop_original->device_copies[0] : NULL;

            if (NULL != pop_cpu_copy && (pop_cpu_copy->flags & PARSEC_DATA_FLAG_DISCARDED)) {
                PARSEC_DEBUG_VERBOSE(20, parsec_gpu_output_stream,
                ...
                assert(pop_cpu_copy->version < gpu_copy->version);
                pop_cpu_copy->version = gpu_copy->version;
                pop_cpu_copy->coherency_state = PARSEC_DATA_COHERENCY_SHARED;
                gpu_copy->coherency_state = PARSEC_DATA_COHERENCY_SHARED;
                assert(PARSEC_DATA_STATUS_UNDER_TRANSFER == pop_cpu_copy->data_transfer_status);
                pop_cpu_copy->data_transfer_status = PARSEC_DATA_STATUS_COMPLETE_TRANSFER;
                if( 0 == (parsec_mpi_allow_gpu_memory_communications & PARSEC_RUNTIME_SEND_GPU_MEMORY) ) {
                    this_task->data[i].data_out = pop_cpu_copy;
                    PARSEC_DEBUG_VERBOSE(100, parsec_gpu_output_stream,
                                     ...
                                     gpu_device->super.device_index, gpu_device->super.name, task_str,
                                     gpu_copy, gpu_copy->super.super.obj_reference_count,
                                     pop_cpu_copy, pop_cpu_copy->super.super.obj_reference_count, __func__);
                } else {
                    parsec_data_protect_cpu_mirror(pop_original);
                }
                PARSEC_DEBUG_VERBOSE(20, parsec_gpu_output_stream,
